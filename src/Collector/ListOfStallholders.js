...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStallholders = async () => {
      ...
      const data = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        paymentStatus: doc.data().paymentStatus || 'Unpaid' // Set default to 'Unpaid'
      }));
  ...
  const handleSearchChange = (event) => {
    ...
    const filteredData = stallHolders.filter(stall => 
      stall.firstName.toLowerCase().includes(searchTerm) ||
      stall.lastName.toLowerCase().includes(searchTerm) ||
      (stall.stallNumber && stall.stallNumber.toString().includes(searchTerm))
    );
